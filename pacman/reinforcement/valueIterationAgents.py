...
class ValueIterationAgent(ValueEstimationAgent):
            ...
            if self.mdp.isTerminal(state):
              ...
              for action in self.mdp.getPossibleActions(state):
                ...
                for succ_state, prob in self.mdp.getTransitionStatesAndProbs(state,action):
                  total += prob * (self.mdp.getReward(state,action,succ_state) + (self.discount*self.values[succ_state]))
                max_val = max(total, max_val)
                temp_value[state] = max_val
    ...
    def computeQValueFromValues(self, state, action):
        ...
        q_total = 0
        for succ_state, prob in self.mdp.getTransitionStatesAndProbs(state,action):
          q_total += prob * (self.mdp.getReward(state,action,succ_state) + (self.discount*self.values[succ_state]))
        return q_total
